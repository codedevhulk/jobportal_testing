          ...
          {jobseeker && <FieldItems style={{ listStyleType: "none", height: "360px" }}>

            {
              Object.entries(jobseeker).map(entry => {
                const label = entry[0]
                const value = entry[1]
                return label !== "jobSeekerId" && label !== "password" ? <FieldItem><Card style={{ ...FieldStyle, textTransform: "uppercase", lineSpacing: "1px" }} >{label}</Card>  <Card style={{ ...FieldStyle, background: "rgba(255,255,255,0.6)", textTransform: "capitalize" }}>{value}</Card> </FieldItem> : null;
              })}
